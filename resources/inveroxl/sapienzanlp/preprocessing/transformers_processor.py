...
from sapienzanlp.preprocessing import MODELS_WITH_DOUBLE_SEP, MODELS_WITH_STARTING_TOKEN
from sapienzanlp.preprocessing.whitespace_tokenizer import WhitespaceTokenizer
...
class TransformersProcessor:
    ...
    def __init__(self, model: Union[str, tr.PreTrainedTokenizer]):
        super(TransformersProcessor, self).__init__()
    ...
    def __call__(
        ...
        text_pair: Union[str, List[str], List[List[str]], List[Word], List[List[Word]]] = None,
        padding: bool = True,
        max_length: int = 0,
        ...
        **kwargs,
        ...
        # if text is str or a list of str or a batch of str, then text needs to be tokenized into `Word`
        # or, if they are not `Word`, they need to be :)
        if (
            isinstance(text, str)
            or isinstance(text[0], str)
            or (is_batched and isinstance(text[0][0], str))
        ):
            text = self.pretokenize(text, is_split_into_words)
            text_pair = self.pretokenize(text_pair, is_split_into_words) if text_pair else None
    ...
    def build_tokens(
        self, text: List[Word], text_pair: List[Word] = None, max_length: int = math.inf
    ...
    def _build_tokens(
        self, text: List[Word], is_b: bool = False, max_length: int = math.inf
        ...
        for word_index, word in enumerate(text):
            ids = self.huggingface_tokenizer(word.text, add_special_tokens=False)["input_ids"]
    ...
    def pretokenize(
        self, text: Union[str, List[str], List[List[str]]], is_split_into_words: bool
    ) -> Union[List[Word], List[List[Word]]]:
        """
        Pre-tokenize the text in input by splitting on spaces. It returns the text
        wrapped in :obj:`Word` objects.
        ...
        Returns:
            :obj:`List[Word]`, :obj:`List[List[Word]]`: The tokenized text.
        """
        return self.tokenizer(text, is_split_into_words)
    ...
    @staticmethod
    def _get_token_type_id(config: transformers.PretrainedConfig) -> int:
        """
